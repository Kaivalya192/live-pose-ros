...
def main():
    rospy.init_node('pose_vis_publisher', anonymous=True)
    
    pose_pub = rospy.Publisher('pose', Float64MultiArray, queue_size=10)
    vis_pub = rospy.Publisher('vis', Image, queue_size=10)
    ...
    try:
        while Estimating and not rospy.is_shutdown():
            ...
            pose_msg.data = center_pose.flatten().tolist()
            pose_pub.publish(pose_msg)

            vis_msg = bridge.cv2_to_imgmsg(vis, "rgb8")
